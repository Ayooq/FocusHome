...
class PageDevicesEdit extends React.Component{
    ...
    update_list(device_id) {
        ...
        util.get({
            ...
            'success': response => {
                ...
                this.setState({inputs: {...this.state.inputs, src: [<option value="" key={-1}>-- не выбрано --</option>].concat(families)}});

                // let aliases = [];
                // aliases.push(<option key={-1} value="">-- не выбрано --</option>);
                // for (let i = 0; i < response.data.aliases.length; i++) {
                //     let item = response.data.aliases[i];
                //     aliases.push(<option key={i} value={ item.id }>{ item.title }</option>)
                // }
                // this.setState({inputs: {...this.state.inputs, aliases: aliases}});
  ...
  render(){
    ...
    gpio_controls.push(<small key={row.gpio__format.controls.length} className="text-primary cur-p"
      ...
      let pin = null;
      if (row.is_pin) {
          pin = <select className="form-control" value={ row.gpio__pin }
                        onChange={ this.unit_property_update.bind(this, this.state.device_unit_id, 'gpio__pin') }>
      ...
      let src = <select className="form-control" value={ row.src_id } onChange={ this.unit_property_update.bind(this, this.state.device_unit_id, 'src_id') } >
          { this.state.inputs.src }
      </select>;
      ...
      let units = <div className="col-md-10 col-lg-8 col-xl-6">
          ...
          <div className="form-group row">
              <label className="col-sm-4 col-form-label">Связанный компонент</label>
              <div className="col-sm-8">
                  { src }
              </div>
          </div>
          <div className="form-group">
              ...
              <div>
                  <label>
                      <input type="checkbox" checked={ row.gpio__format.is_default }
                             onChange={this.device_gpio_format_update.bind(this, this.state.device_unit_id, 'is_default')}/>
              ...
              <div>
                  ...
                  <div className="form-group row">
                      ...
                      <div className="col-sm-8">
                          { row_values }
                          <div>
                              <small>Для указания диапозона значений, указывать в формате <code>-100..20</code> (от -100
                                  до 20)
                              </small>
                          </div>
                      </div>
