...
class PageIndex extends React.Component{
    constructor(props) {
        ...
        this.devices_monitoring = this.devices_monitoring.bind(this);
    }
    ...
    componentDidMount() {
        ...
        this.context.emit('subscribe:devices_monitoring', {});
    }

    componentWillUnmount() {
        //clearTimeout(this.state.timer_id);
        this.context.emit('unsubscribe:devices_monitoring', {});
    }

    devices_monitoring(msg){
        console.log('/pages/monitoring/index.jsx::socket message');

        let device_id = msg.device_id;
        let unit_format = msg.unit_format;
        let family = Object.keys(unit_format)[0];
        
        if (family != undefined) {
            let unit = Object.keys(unit_format[family])[0];
            
            if (device_id in this.state.data.data) {
                if (family in this.state.data.data[device_id].status) {
                    if (unit in this.state.data.data[device_id].status[family]) {
                        this.setState((state) => {
                            state.data.data[device_id].status[family][unit] = unit_format[family][unit];
                            return state;
                        });
                    }
                }
    ...
    render() {
            ...
            for (let key in list) {
                ...
                if ("temp" in list[key].status) {
                    for (let temp_key in list[key].status.temp) {
                        ...
                        temps[temp_key] =
                            <div key={temp_key} className={ "device-ins-block-small " + t.unit_format.class }
                                 title={ t.unit_format.title }>{ t.unit_format.caption }</div>
                    }
                }
                ...
                if ("ins" in list[key].status) {
                    for (let ins_key in list[key].status.ins) {
                        ...
                        ins.push(<div key={ins_key} className={ "device-ins-block-small " + t.unit_format.class }
                                      title={ t.unit_format.title }>{ t.unit_format.title }: { t.unit_format.caption }</div>)
                    }
                }
                ...
                if ("couts" in list[key].status) {
                    for (let couts_key in list[key].status.couts) {
                        ...
                        couts.push(<div key={couts_key} className={ "device-ins-block-small " + t.unit_format.class }
                                        title={ t.unit_format.title }>{ t.unit_format.title }: { t.unit_format.caption }</div>)
                    }
                }
                ...
                devices.push(<tr key={list[key].id} className="tr-device">
                    ...
                    <td>
                        ...
                        <small>{list[key].client__name}</small>
                    </td>
        ...
        return <div className="page-content container-fluid">
            <Event event='devices_monitoring' handler={this.devices_monitoring}/>
            <div className="row">
                <BGC title={block_title} buttons={buttons} col="col-md-12">
                    <div className="form-group">
                        ...
                        <div>
                            ...
                            <span>&nbsp;</span>
                            <button onClick={ this.update_list.bind(this) } className="btn btn-primary btn-sm"><i
                                className="ti-reload"></i> найти
                            </button>
                        </div>
                        }
                    </div>
...
PageIndex.contextType = ReactSocketIO.SocketContext;
