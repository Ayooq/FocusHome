    ...
    if ("data" in this.state.data) {
      ...
      if ("ins" in this.state.data.data.status) {
        for (let key in this.state.data.data.status.ins) {
          ...
          let ctrl = t.unit_format.control || null;
          let ctrlBtn = null;
          if (ctrl === "toggle"){
            ctrlBtn = <span className="float-left btn-toggle" onClick={ this.unitToggle.bind(this, this.state.data.data.id, t.unit__id) }><i className="ti-power-off"></i></span>
          }
          ...
          ins.push(<div key={key} className="device-ins-block">
            <div className={ "p-10 " + t.unit_format.class }>
              ...
              <div>
                ...
                <div className="text-right">{ ctrlBtn } { t.date }</div>
              </div>
            </div>
          </div>)
